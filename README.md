...
### 🌙 밤 모드 — 취하기 전에 켜두는 기록
- 지금까지의 추적은 "위급하다고 판단한 사람이 켜는 것"을 전제로 했다.
  **취한 사람은 그 판단을 못 한다.** 그래서 판단이 멀쩡할 때 한 번 켜두고
  잊어버리는 모드를 따로 뒀다
- 종료 시각(새벽 4/6/8시)만 고르면 끝. **취한 사람은 종료도 안 누르므로**
  시각이 되면 스스로 마친다
- 주기 10분 — 밤새 돌아야 하므로 배터리가 관건
- **문자를 보내지 않는다** — 공유가 아니라 기록이 목적. 필요하면 추적
  화면에서 언제든 링크를 보낼 수 있다

### 📷 사진 타임라인 — 좌표 대신 기억
- 좌표는 기억을 되살리지 못한다. `37.5632, 126.9851 에 40분 머무름` 은
  아무 말도 안 해주지만, **"1:40에 여기서 사진 3장"** 은 그 자리를 떠올리게 한다
- 세션 시간대에 찍은 사진의 **촬영 시각과 GPS**를 읽어 체류 구간과 하나의
  타임라인으로 병합. 사진에 위치가 없으면 촬영 시각으로 동선의 어느 지점인지 추정
- **사진은 기기 밖으로 나가지 않는다.** 복사하지도, 서버에 올리지도 않고
  시각·좌표·미리보기 URI 만 로컬에서 읽는다
- 권한 거부·일부 허용(iOS)·웹에서는 조용히 생략하고 이유를 알려줌
...
│   │   ├── nightMode.js         ← 밤 모드 종료 시각 계산
│   │   ├── photos.js            ← 사진 시각·위치 읽기 + 타임라인 병합
